...
from bitmovin_api_sdk import S3RoleBasedOutput
...
# Example H.264 video encoding profiles (live, role-based S3 output).
video_encoding_profiles = [
    ...
    dict(height=1080, bitrate=8000000, profile=ProfileH264.HIGH, level=LevelH264.L4, mode=StreamMode.STANDARD)
]
...
def main():
    """
    Main entry point for the role-based S3 live encoding script.

    1. Fetch an RTMP input from your Bitmovin account and create a Role-Based S3 Output.
    2. Create an Encoding resource and configure it for the AWS AP_NORTHEAST_1 region.
    3. For each of the H.264 video profiles:
       - Set up color configuration (copy color flags from source).
       - Customize advanced parameters like CABAC, reference frames, etc.
       - Create a video stream from the RTMP input (position=0).
       - Create an FMP4 muxing, stored in the role-based S3 output.
    4. Do the same for each AAC audio profile (position=1).
    5. Prepare HLS and DASH manifests for live streaming, set up timeshift, offset, etc.
    6. Launch the live encoding with a stream key. Wait until RUNNING state before proceeding.
    7. Print the RTMP endpoint and stream key, and pause until user input.
    8. Stop the live encoding and wait for it to finish.
    """
    ...
    for video_profile in video_encoding_profiles:
        ...
        video_muxing_output = EncodingOutput(
            ...
            acl=[AclEntry(permission=AclPermission.PRIVATE)]
        )
    ...
    # Build live-specific HLS/DASH manifest configurations
